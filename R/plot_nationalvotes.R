...
plot_nationalvotes <- function(votedate = NULL, vote_id = NULL, geolevel = "municipality", measure = "result", 
                               standardize = T, lakes = T, theme = "srf") {
  
  # Check inputs
  if (!geolevel %in% c("canton", "district", "municipality", "zh_counting_districts")) stop("Please select valid 'geolevel'.")
  ...
  # Fetch vote data
  vote_data <- get_nationalvotes(geolevel = geolevel, votedates = votedate)
  ...
  # Base plot
  plot_map_national(pd, lakes = lakes_data, legend_title = legend_title, theme = theme)
